  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.message.includes('Invalid login credentials')) {
        setAuthError('이미 구글 등 다른 방식으로 가입된 이메일입니다. 다른 테스트 이메일을 입력하거나 아래의 구글 로그인을 이용해주세요.');
      } else {
                  ...
                  <div className="flex-1 flex flex-col items-center text-center max-w-4xl mx-auto w-full pt-8 pb-16 custom-scrollbar">
                    ...
                    <div className="w-full max-w-md bg-zinc-900/80 p-8 rounded-3xl border border-zinc-800 shadow-2xl backdrop-blur-sm flex flex-col gap-4 relative mx-auto">
                      ...
                      <div className="flex items-center gap-3 w-full my-1">
                        ...
                        <span className="text-xs font-medium text-zinc-500">또는</span>
                        <div className="h-px bg-zinc-800 flex-1"></div>
                      </div>
                      
                      <button 
                        onClick={() => supabase.auth.signInWithOAuth({ provider: 'google' })}
                        className="w-full bg-white text-black font-bold py-3.5 rounded-xl hover:bg-zinc-200 transition-all active:scale-95 shadow-lg flex justify-center items-center gap-2"
                      >
